...
TEST_CASE("LoadChart compiles SUS fixtures end to end", "[SheetsAnalyzer][Chart][SUS][E2E]") {
    ...
    SECTION("directional notes generated by an editor") {
        const auto result = xlair::sheets::LoadChart(SusFixture(U"directional.sus"), options);

        INFO(FirstDiagnostic(result));
        REQUIRE(result);
        CHECK(result->slider_notes.isEmpty());
        REQUIRE(result->side_notes.size() == 4);
        CHECK(result->side_notes[0].button == xlair::sheets::SideButton::LeftUpper);
        CHECK(result->side_notes[1].button == xlair::sheets::SideButton::LeftLower);
        CHECK(result->side_notes[2].button == xlair::sheets::SideButton::RightLower);
        CHECK(result->side_notes[3].button == xlair::sheets::SideButton::RightUpper);
        CHECK(result->total_combo == 4);
    }
}

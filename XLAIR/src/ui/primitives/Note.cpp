...
namespace ui::primitives {
    void DrawTapNote(const Vec2& left_center, double width, double height) {
        ...
        RoundRect{ left_center.movedBy(-height / 2, 0), height, width, 5.0 }
            .stretched(0, -5)
    ...
    void DrawHoldBack(const Vec2& start_left, double start_width, double start_t, const Vec2& end_left, double end_width, double end_t) {
        static constexpr ColorF StartColor{ U"#EC43F240" };
        static constexpr ColorF EndColor{ U"#43A4F250" };

        const auto start_color = Math::Lerp(StartColor, EndColor, start_t);
        const auto end_color = Math::Lerp(StartColor, EndColor, end_t);
        Quad{
            start_left.movedBy(5, 0),
            start_left.movedBy(start_width - 10, 0),
            end_left.movedBy(end_width - 10, 0),
            end_left.movedBy(5, 0),
        }.draw(
            start_color,
            start_color,
            end_color,
